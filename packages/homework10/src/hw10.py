...
def ActionClient(n):
    ...
    rospy.logwarn("Action for " +str(n)+ "Send Time is " +str(Send_Time))
    Wait_Time = ActT3 - ActT2
    rospy.logwarn("Action" +str(n)+ "Wait Time is " +str(Wait_Time))
    return client.get_result()
if __name__=="__main__":
    rospy.init_node('hw10')
    Serv3 = ServiceClient(3)
    rospy.logwarn("Service Order 3 is " +str(Serv3))
